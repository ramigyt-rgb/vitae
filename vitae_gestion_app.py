...
def restore_table_from_sheet(table: str) -> int:
    ...
    Baja una pestaña de Google Sheets a SQLite.

    Si la pestaña está vacía o dice Sin datos, NO pisa la base local.
    ...
    try:
        ...
        st.warning(f"No existe la pestaña {table} en Google Sheets.")
    ...
    if not values:
        ...
        st.warning(f"{table}: Google Sheets vacío. No se pisa SQLite.")
    ...
    if len(values) == 1:
        ...
        if str(first_value).strip().lower() == "sin datos":
            ...
            st.warning(f"{table}: figura 'Sin datos'. No se pisa SQLite.")
    ...
    if df.empty:
        ...
        st.warning(f"{table}: DataFrame vacío. No se pisa SQLite.")
...
def add_balance_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "importe_usd" in df.columns and "pagado_usd" in df.columns:
        df["saldo_usd"] = df["importe_usd"].apply(money) - df["pagado_usd"].apply(money)
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            total_usd = filtered["importe_usd"].apply(money).sum() if "importe_usd" in filtered.columns else 0
            pagado_usd = filtered["pagado_usd"].apply(money).sum() if "pagado_usd" in filtered.columns else 0
            ...
            if module_name in ["Cuenta Corriente VMR", "Cuenta Corriente VM"]:
                c1.metric("💰 Facturado", f"{fmt_money(total_facturado)} | USD {total_usd:,.2f}")
                c2.metric("💸 Pagado", f"{fmt_money(cobrado)} | USD {pagado_usd:,.2f}")
                c3.metric("💰 A pagar", f"{fmt_money(pendiente)} | USD {pendiente_usd:,.2f}")
            else:
...
def render_dashboard() -> None:
    ...
    def deuda_usd_mod(nombre):
        ...
        if "saldo_usd" in df.columns:
            return df["saldo_usd"].apply(money).sum()
        if "importe_usd" in df.columns:
            pagado_usd = df["pagado_usd"].apply(money) if "pagado_usd" in df.columns else 0
            return max(0, df["importe_usd"].apply(money).sum() - pagado_usd.sum())
        return 0.0
