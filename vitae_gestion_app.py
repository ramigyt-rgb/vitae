...
def save_table_changes(table: str, edited_df: pd.DataFrame, allow_delete: bool = False) -> int:
    ...
    with connect() as conn:
        ...
        current_ids = set(current_df["id"].astype(int).tolist()) if "id" in current_df.columns and not current_df.empty else set()
        updated = 0
        inserted = 0
        deleted = 0
        conn.execute("BEGIN")
...
def render_caja_pro_panel(df: pd.DataFrame, module_name: str) -> None:
    ...
    if "observaciones" in data.columns:
        resp = data.copy()
        resp["observaciones"] = resp["observaciones"].astype(str).str.strip()
        resp = resp[resp["observaciones"] != ""]
        if not resp.empty:
            por_resp = (
                resp.groupby("observaciones", as_index=False)[["ingreso", "egreso"]]
                .sum()
                .sort_values("egreso", ascending=False)
                .head(10)
            )
            fig_resp = px.bar(
                ...
                y=["ingreso", "egreso"],
                barmode="group",
                title="Movimientos por responsable / observación",
            )
            fig_resp.update_layout(
                xaxis_title="Responsable / Observación",
                yaxis_title="Importe",
                height=420,
            )
            g2.plotly_chart(fig_resp, use_container_width=True)
# =========================================================
