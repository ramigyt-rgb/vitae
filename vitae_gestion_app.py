...
MODULES: Dict[str, Dict[str, Any]] = {
    ...
    "Cuenta Corriente VM": {
        ...
        "fields": [
            ...
            ("pagado_usd", "money", False),
            ("vencimiento", "date", False),
            ("Saldo", "select", True, ["Pendiente", "Parcial", "Pagado", "Vencido"]),
            ("observaciones", "textarea", False),
        ],
    },
    "Facturación VM": {
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            if table == "Cuenta Corriente VMR":
                ...
                c1.metric("💰 Facturado ARS", fmt_money(total_facturado))
                c2.metric("💸 Pagado ARS", fmt_money(cobrado))
                c3.metric("⏳ A pagar ARS", fmt_money(pendiente))
            else:
                c1, c2, c3, c4 = st.columns(4)
