...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            col_monto = "valor_pesos" if "valor_pesos" in filtered.columns else "importe" if "importe" in filtered.columns else "monto" in filtered.columns else "saldo" if "saldo" in filtered.columns else None
...
def render_resumen_empresa(titulo, empresa):
        ...
        if resumen.empty:
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"grafico_modulos_{titulo}_{empresa}")
        st.divider()                                
