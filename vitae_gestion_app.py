...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab1:
        ...
        st.subheader("Indicadores de facturación")
        ...
        c1.metric("Registros visibles", registros)



        c2.metric("Total facturado", fmt_money(total_facturado))



        c3.metric("Total USD", f"USD {total_usd:,.2f}")
        ...
        if fecha_col and "valor_pesos" in filtered.columns:
            ...
            if not graph.empty:
                ...
                chart = graph.groupby(graph[fecha_col].dt.date)["valor_pesos"].sum().reset_index()
                ...
                fig = px.line(
                    ...
                    title=f"Evolución de facturación - {module_name}",
    ...
    with tab4:
        ...
        if df.empty:
            ...
            export_df = module_business_df(df, cfg)
