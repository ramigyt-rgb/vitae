...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_editar:
        ...
        if df.empty:
            ...
            if table == "cuenta_corriente_vm":
                df_edit = df_edit.drp(columns=["pagado_usd"], errors="ignore")
            ...
            edited_df = st.data_editor(
                ...
                num_rows="dynamic",
                column_config={
                    "importe_usd": st.column_config.NumberColumn(
                        "Importe USD",
                        step=0.01,
                        format="%.2f",
                    ),
                    "pagado_usd": st.column_config.NumberColumn(
                        "Pagado USD",
                        step=0.01,
                        format="%.2f",
                    ),
    ...
    with tab_exportar:
        df = add_balance_columns(get_df(table))
        if table == "cuenta_corriente_vm":
            df = df.drop(columns=["pagado_usd"], errors="ignore")
        if df.empty:
