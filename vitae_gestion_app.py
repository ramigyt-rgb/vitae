...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ...
        ["📊 Panel PRO", "➕ Cargar", "📥 Importar", "🏷️ Editar columnas", "📤 Exportar"]
    ...
    with tab1:
        ...
        if df.empty:
            ...
            st.warning("Todavía no hay registros cargados.")
        ...
        work["fecha_factura_dt"] = pd.to_datetime(work.get("fecha_factura", ""), errors="coerce")



        work["vencimiento_dt"] = pd.to_datetime(work.get("vencimiento", ""), errors="coerce")



        estado_norm = work["estado"].astype(str).str.lower().str.strip()



        total = work["valor_pesos"].apply(money).sum()



        cobrado = work[estado_norm.isin(["completo", "cobrado", "pagado"])]["valor_pesos"].apply(money).sum()
        ...
        vencido = work[
            ...
            work["vencimiento_dt"].notna()



            & (work["vencimiento_dt"] < pd.Timestamp.today().normalize())



            & (~estado_norm.isin(["completo", "cobrado", "pagado", "anulado"]))
        ...
        c1.metric("Total facturado", fmt_money(total))



        c2.metric("Cobrado estimado", fmt_money(cobrado))
        ...
        c4.metric("Vencido", fmt_money(vencido))



        c5.metric("Registros", len(work))
        ...
        buscar = f1.text_input("Buscar paciente / factura")



        obra = f2.multiselect("Obra social", sorted(work["obra_social"].dropna().astype(str).unique()))



        medico = f3.multiselect("Médico", sorted(work["medico_responsable"].dropna().astype(str).unique()))



        estado = f4.multiselect("Estado", sorted(work["estado"].dropna().astype(str).unique()))
        ...
        proc = f5.multiselect("Procedimiento", sorted(work["procedimiento"].dropna().astype(str).unique()))



        mes = f6.multiselect("Mes", sorted(work["mes"].dropna().astype(str).unique()))



        desde = f7.date_input("Desde", value=date.today() - timedelta(days=3650))



        hasta = f8.date_input("Hasta", value=date.today() + timedelta(days=3650))
        ...
        if buscar:
            ...
            filtered = filtered[
                ...
                filtered.astype(str).apply(lambda col: col.str.contains(buscar, case=False, na=False)).any(axis=1)
        ...
        if obra:
            ...
            filtered = filtered[filtered["obra_social"].astype(str).isin(obra)]
        ...
        if medico:
            ...
            filtered = filtered[filtered["medico_responsable"].astype(str).isin(medico)]
        ...
        if estado:
            ...
            filtered = filtered[filtered["estado"].astype(str).isin(estado)]
        ...
        if proc:
            ...
            filtered = filtered[filtered["procedimiento"].astype(str).isin(proc)]
        ...
        if mes:
            ...
            filtered = filtered[filtered["mes"].astype(str).isin(mes)]
        ...
        st.subheader("Tabla profesional")



        cols = [c for c in get_field_names(cfg) if c in filtered.columns]
        ...
        st.dataframe(rename_fact_df(show_df, labels), use_container_width=True, hide_index=True)
        ...
        graph["fecha_factura_dt"] = pd.to_datetime(graph["fecha_factura"], errors="coerce")



        graph = graph[graph["fecha_factura_dt"].notna()]
        ...
        if not graph.empty:
            ...
            graph["periodo"] = graph["fecha_factura_dt"].dt.to_period("M").astype(str)



            mensual = graph.groupby("periodo", as_index=False)["valor_pesos"].sum()



            fig1 = px.bar(mensual, x="periodo", y="valor_pesos", title="Facturación mensual")
        ...
        if not filtered.empty:
            ...
            por_os = filtered.groupby("obra_social", as_index=False)["valor_pesos"].sum().sort_values("valor_pesos", ascending=False).head(12)



            fig2 = px.bar(por_os, x="obra_social", y="valor_pesos", title="Facturación por obra social")
    ...
    with tab4:
        ...
        st.subheader("Editar títulos visibles")



        st.caption("Esto cambia cómo se ven las columnas en pantalla. No rompe la base de datos.")



        with st.form(f"labels_{table}"):
            ...
            if st.form_submit_button("Guardar títulos", type="primary"):
    ...
    with tab5:
        ...
        if df.empty:
            ...
            export_df = rename_fact_df(df[[c for c in get_field_names(cfg) if c in df.columns]], labels)
            ...
            st.download_button("Descargar CSV PRO", csv, file_name=f"{table}_pro.csv", mime="text/csv")
...
def render_module(module_name: str) -> None:
    ...
    if module_name in ["Facturación VMR", "Facturacion VM"]
