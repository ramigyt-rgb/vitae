...
def sync_table_to_sheet(table: str):
    ...
    df = get_df(table)
    st.write("Tabla:", table)
    st.write("Filas locales:", len(df))
    gc = get_gs_client()
    st.write("Cliente Google OK")
    sh = gc.open_by_key(SHEET_ID)
    st.write("Sheet Abierto:", sh.title)
    try:
        ws = sh.worksheet(table)
        st.write("Pestaña encontrada:", table)
    ...
    ws.update(data)
    st.success(f"Sincronizado :{table}: {len(df)} filas")
...
def render_configuracion() -> None:
    ...
    with col1:
        if st.button("⬆️ Subir datos actuales a Google Sheets"):
            try:
                sync_all_to_sheets()
                st.success("Google Sheets actualizado con los datos actuales de la app.")
            except Exception as e:
                st.error(f"No se pudo subir a Google Sheets: {e}")
    with col2:
