...
def get_or_create_worksheet_cached(table: str):
...
def read_table_from_sheet(table: str) -> pd.DataFrame:
    ...
    ws = get_or_create_worksheet_cached(table)
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        df = add_balance_columns(read_table_from_sheet(table))
        filtered = df.copy()
        if df.empty:
    ...
    with tab_editar:
        ...
        df = read_table_from_sheet(table)
