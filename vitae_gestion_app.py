...
def first_available_date_col(df: pd.DataFrame, module_name: str) -> str | None:
    if module_name in ["facturacion_vmr", "facturacion_vm"] and "fecha_factura" in df.columns:
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with connect() as conn:
        ...
        conn.execute("DROP TABLE IF EXISTS facturacion_vm")

        conn.execute("""

            CREATE TABLE facturacion_vm (
