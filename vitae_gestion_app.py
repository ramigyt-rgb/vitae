...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_editar:
        ...
        if df.empty:
            ...
            if guardar:
                editor_state = st.session_state.get(f"editor_{table}", {})
                ...
                if deleted_rows:
                    ...
                    st.success(f"Filas eliminadas: {len(deleted_rows)}")
                    st.rerun()
                else:    
                    limpio = edited_df.copy() 

                if "id" in limpio.columns:
                    for _, row in limpio.iterrows():
                        row_id = row.get("id")
                        if pd.notna(row_id):
                            data = row.drop(labels=["id"], errors="ignore").to_dict()
                            update_row(table, int(row_id), data)                                                  
