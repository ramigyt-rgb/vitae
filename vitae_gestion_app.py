...
def format_facturacion_table(df: pd.DataFrame, labels: Dict[str, str]) -> pd.DataFrame:
    ...
    show = show[[field[0] for field in cfg["fields"] if field[0] in show.columns]].copy()
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            st.markdown("### Filtros profesionales")
            ...
            with f1:
                ...
                buscar = st.text_input("Buscar paciente / factura / observación", key=f"buscar_pro_{table}")
            ...
            with f2:
                ...
                obras = ["Todas"] + sorted(df["obra_social"].dropna().astype(str).unique().tolist()) if "obra_social" in df.columns else ["Todas"]



                obra = st.selectbox("Obra social", obras, key=f"obra_pro_{table}")
            ...
            with f3:
                ...
                procedimientos = ["Todos"] + sorted(df["procedimiento"].dropna().astype(str).unique().tolist()) if "procedimiento" in df.columns else ["Todos"]



                procedimiento = st.selectbox("Procedimiento", procedimientos, key=f"proc_pro_{table}")
            ...
            with f4:
                ...
                medicos = ["Todos"] + sorted(df["medico_responsable"].dropna().astype(str).unique().tolist()) if "medico_responsable" in df.columns else ["Todos"]



                medico = st.selectbox("Médico responsable", medicos, key=f"medico_pro_{table}")
            ...
            with f5:
                ...
                estados = ["Todos"] + sorted(df["estado"].dropna().astype(str).unique().tolist()) if "estado" in df.columns else ["Todos"]



                estado = st.selectbox("Estado", estados, key=f"estado_pro_{table}")
            ...
            with f6:
                ...
                fecha_desde = st.date_input("Desde", value=date.today() - timedelta(days=3650), key=f"desde_pro_{table}")



                fecha_hasta = st.date_input("Hasta", value=date.today() + timedelta(days=3650), key=f"hasta_pro_{table}")
