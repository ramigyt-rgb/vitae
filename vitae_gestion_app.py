...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            if "estado" in filtered.columns and "pagado_usd" in filtered.columns:
                pagado_usd = filtered[
                    filtered["estado"].astype(str).str.lower().str.strip().isin(["completo", "pagado", "cobrado"])
                ]["pagado_usd"].apply(money).sum()
            elif "pagado_usd" in filtered.columns:
                pagado_usd = filtered["pagado_usd"].apply(money).sum()
            else:
                pagado_usd = 0
