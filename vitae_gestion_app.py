    ...
    for name, df in mods.items():
        ...
        if "valor_pesos" in df.columns:
            ...
            if "estado" in df.columns:
                ...
                estado = df["estado"].astype(str).str.lower()

                cobrado += df[

                    estado.isin(["completo", "cobrado", "pagado"])

                ]["valor_pesos"].apply(money).sum()

                a_cobrar += df[

                    estado.isin(["pendiente", "parcial", "vencido"])
        ...
        if "monto" in df.columns and "estado" in df.columns:
            ...
            a_pagar_emp += df[
                ...
                estado.isin(["pendiente", "vencido"])

            ]["monto"].apply(money).sum()
        ...
        if name == "Tareas Pendientes" and "estado" in df.columns:
            ...
            tareas_emp += len(
                ...
                df[~estado.isin(["finalizada", "cancelada"])]
...
for name, cfg in MODULES.items():
    ...
    temp["vencimiento_dt"] = pd.to_datetime(
    ...
    temp = temp[temp["vencimiento_dt"] >= pd.Timestamp(hoy)]
...
if venc_df.empty:
    ...
    st.dataframe(
        ...
        venc_df.sort_values("Vencimiento"),
