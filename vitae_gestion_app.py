...
# BASE DE DATOS GOOGLE SHEETS ONLY

# =========================================================

def get_cfg_by_table(table: str) -> Dict[str, Any]:
    ...
    raise Exception(f"No existe configuración para tabla: {table}")

def expected_columns(table: str) -> List[str]:

    cfg = get_cfg_by_table(table)

    return ["id", "created_at", "updated_at"] + [f[0] for f in cfg["fields"]]

def init_db() -> None:

    # SQLite desactivado. No hace nada.
...
def get_or_create_worksheet(sh, table: str):
    ...
    try:
        ...
        ws = sh.add_worksheet(title=table, rows=1000, cols=40)

        ws.update([expected_columns(table)])
...
def read_table_from_sheet(table: str) -> pd.DataFrame:
    ...
    if len(header) == 1 and str(header[0]).strip().lower() == "sin datos":
...
def write_table_to_sheet(table: str, df: pd.DataFrame) -> int:
...
def next_sheet_id(df: pd.DataFrame) -> int:
...
def bulk_insert_rows(table: str, rows: List[Dict[str, Any]]) -> int:
    ...
    nuevo = pd.DataFrame(clean_rows)

    final = pd.concat([df, nuevo], ignore_index=True)

    # Evita duplicados reales por contenido, ignorando columnas técnicas

    business_cols = [c for c in final.columns if c not in ["id", "created_at", "updated_at"]]

    final = final.drop_duplicates(subset=business_cols, keep="first")

    write_table_to_sheet(table, final)
...
def save_table_changes(table: str, edited_df: pd.DataFrame, allow_delete: bool = False) -> int:
    ...
    if current.empty:
        ...
        edited["created_at"] = now

        edited["updated_at"] = now

        write_table_to_sheet(table, edited)

        return len(edited)

    current["id"] = pd.to_numeric(current["id"], errors="coerce")
    ...
    for _, row in edited.iterrows():
        ...
        if pd.isna(row_id):
            ...
            mask = current["id"].astype("Int64") == row_id
            ...
            if mask.any():
                ...
                for c in edited.columns:
                    ...
                    if c != "created_at":
                        ...
                        current.loc[mask, c] = row[c]

                current.loc[mask, "updated_at"] = now
    ...
    write_table_to_sheet(table, current)

    return updated + inserted

def replace_table_rows(table: str, rows: List[Dict[str, Any]]) -> int:

    return bulk_insert_rows_safe_replace(table, rows)
...
def sync_df_to_sheet(table: str, df: pd.DataFrame) -> int:
    ...
    return write_table_to_sheet(table, df)
...
def sync_all_to_sheets() -> Dict[str, int]:
    ...
    return {cfg["table"]: len(get_df(cfg["table"])) for cfg in MODULES.values()}

def restore_table_from_sheet(table: str) -> int:

    return len(get_df(table))
        ...
        if df.empty:
            ...
            if guardar:
                try:
                    ...
                    total_guardado = save_table_changes(
                        table=table,
                        edited_df=limpio,
                        allow_delete=True
                    )
