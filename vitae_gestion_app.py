...
def format_facturacion_table(df: pd.DataFrame, labels: Dict[str, str]) -> pd.DataFrame:
    ...
    show = df[cols].copy()
    for col in ["mes", "fecha_factura", "vencimiento", "fecha_pago"]:
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            tabla = format_facturacion_table(filtered, labels)
            columnas_visibles = [
                "Mes",
                "Paciente / Afiliado",
                "Obra social",
            ...
            tabla = tabla[[c for c in columnas_visibles if c in tabla.columns]]
            st.dataframe(
                tabla,
                ...
                column_config={
                    ...
                    "Paciente / Afiliado": st.column_config.TextColumn("Paciente / Afiliado", width="large"),
                    "Obra social": st.column_config.TextColumn("Obra social", width="medium"),
                    "Procedimiento": st.column_config.TextColumn("Procedimiento", width="medium"),
                    "Médico": st.column_config.TextColumn("Médico", width="small"),
                    "Fecha factura": st.column_config.TextColumn("Fecha factura", width="small"),
                    "N° factura": st.column_config.TextColumn("N° factura", width="small"),
                    "Vencimiento": st.column_config.TextColumn("Vencimiento", width="small"),
                    "Fecha pago": st.column_config.TextColumn("Fecha pago", width="small"),
                    "Valor facturado": st.column_config.TextColumn("Valor facturado", width="medium"),
