...
def render_analisis_mensual_2026(df: pd.DataFrame):
    ...
    col1.metric("Facturación 2026", format_money(acumulado))

    col2.metric("Promedio mensual", format_money(promedio))

    col3.metric("Mejor mes", mejor_mes["mes_nombre"])

    col4.metric("Proyección anual", format_money(proyeccion))
        ...
        for name, df in mods.items():
            ...
            if "valor_pesos" in df.columns:
                ...
                if "estado" in df.columns:
                    ...
                    cobrado +=
df[df["estado"].astype(str).str.lower().isin(["completo", "cobrado",
"pagado"])]["valor_pesos"].apply(money).sum()

                    a_cobrar +=
df[df["estado"].astype(str).str.lower().isin(["pendiente", "parcial",
"vencido"])]["valor_pesos"].apply(money).sum()

            if "monto" in df.columns and "estado" in df.columns:

                a_pagar_emp +=
df[df["estado"].astype(str).str.lower().isin(["pendiente",
"vencido"])]["monto"].apply(money).sum()
            ...
            if name == "Tareas Pendientes" and "estado" in df.columns:
                ...
                tareas_emp +=
len(df[~df["estado"].astype(str).str.lower().isin(["finalizada",
"cancelada"])])
...
st.markdown("### Facturación por módulo")
    ...
    for name, df in dfs.items():
        if df.empty or "vencimiento" not in df.columns:
            continue
        temp = df.copy()
        temp["vencimiento_dt"] = pd.to_datetime(temp["vencimiento"],
errors="coerce")
        ...
        for _, row in temp.iterrows():
            venc_rows.append({
                ...
                "Detalle": row.get("concepto") or row.get("detalle")
or row.get("tarea") or row.get("acreedor") or row.get("afiliado") or
row.get("procedimiento") or "",
                "Obra Social": row.get("obra_social") or row.get("obra
social") or row.get("os") or "",
                "Importe": row.get("importe") or row.get("saldo") or
row.get("valor") or row.get("valor_pesos") or 0,
    ...
    if venc_df.empty:
        ...
        st.dataframe(venc_df.sort_values("Vencimiento"),
...
def render_admin() -> None:
    ...
    if st.button("Sincronizar todo con Google Sheets"):

        sync_all_to_sheets()

        st.success("Todos los módulos fueron sincronizados con Google Sheets.")
